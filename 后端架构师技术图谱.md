...
* [《Java Set集合的详解》](https://blog.csdn.net/qq_33642117/article/details/52040345)
  ...
  * TreeSet：红黑树结构；元素实现Comparable接口保证可排序，或者传入Comparator使用容器排序(优先级更高)
...
* [《Java BitSet（位集）》](https://blog.csdn.net/caiandyong/article/details/51581160)
  ...
  * 使用long数组原因：为了and和or操作（需要遍历全部数据）减少循环次数，提高性能
...
* [《十种常见排序算法的思想、应用场景》](https://www.jianshu.com/p/47170b1ced23)
  ...
  * 特殊要求：计数排序（非负数），基数排序（位数一致且数字），桶排序（数据均匀分布，0~1之间）
...
* [《排序算法（三）：计数排序与桶排序》](https://blog.csdn.net/sunjinshengli/article/details/70738527)
	...
	* 桶排序：将[0，1)区间划分为n个相同的大小的子区间（通），单独进行排序，然后再遍历每个桶；要求均匀分布；
...
+ 优化-分治：分组后分别构建堆；优化-去重：Hash去重重复数
+ 实际常见：使用MapReduce框架
...
+ [《逃逸分析与标量替换》](https://mp.weixin.qq.com/s?__biz=MzI3ODcxMzQzMw==&mid=2247492444&idx=2&sn=f8d8a9cc4b2e66da2d532a562dda2bae)
  ...
  + 两者都是为了减少对象分配
+ [《一文读懂java中的Reference和引用类型》](https://www.cnblogs.com/flydean/p/java-reference-referencetype.html)
  + 强：不会被回收，软：内存不足时回收，弱：必定回收，虚：回收通知
...
- [《JDK特性解读》](https://pdai.tech/md/outline/x-outline.html#java-8-%E6%96%B0%E7%89%B9%E6%80%A7%E8%A7%A3%E8%AF%BB)
  - 8：函数编程、Optional类、重复注解、LocalDateTime
  - 9：反应式流、统一JVM日志、Java平台模块系统、进程API
  - 10：局部变量推断、统一垃圾回收接口、G1优化、数据共享、线程管控
...
* [《彻底理解synchronized》](https://juejin.cn/post/6844903600334831629)
  ...
  + synchronized实现：JMM层面提供lock/unlock操作，虚拟机层面的monitor对象锁机制使用两类操作
    + 1：enter->成功->执行->exit
  ...
  + 只升级不降级：避免消耗，获得锁的线程被阻塞住了（降级专指写锁降级为读锁）

#### final

* [《你以为你真的了解final吗？》](https://juejin.cn/post/6844903601068998664)
  + 方法可以重载不可以重写；类变量可以在声明和静态放方块进行赋值；成员变量可以在声明、非静态放方块和构造方法进行赋值；类不可以继承
...
+ [《并发容器之ConcurrentHashMap(JDK 1.8版本)》](https://juejin.cn/post/6844903602423595015)
  ...
  + JDK8之后：CAS+Synchronized，Node替换segment，对Node加锁；并发场景下put会检查节点状态，辅助扩容
...
* [《JAVA中写时复制(Copy-On-Write)Map实现》](https://www.cnblogs.com/hapjin/p/4840107.html)
  * 实现读写分离，读取发生在原始数据上，写入（加锁）发生在副本上。  
...
+ [《并发容器之ThreadLocal》](https://juejin.cn/post/6844903602436177933)
  + 以ThreadLocal对象自身为key放入ThreadLocalMap中
  + ThreadLocalMap是一个Entry数组；Entry继承了WeakReference弱引用；ThreadLocalMap 使用**开放地址法**解决散列冲突
  + GC时Entry.key会被回收，从而出现Entry(null，value)的数据，因使用线程池一般线程不会被结束，从而Entry强引用一直存在导致内存泄露
...
* [《Redis底层存储模型》](https://i6448038.github.io/2019/12/01/redis-data-struct/)
  * 根据长度(39、64）、元素个数（128、512）确定类型：字符串[int、sds]，列表[ziplist、双向链表]，hash[ziplist、哈希表]，set[intset、哈希表]，zset[ziplist、跳表]
...
* [《Redis 内存回收策略和key失效机制》](https://juejin.cn/post/6844904194751594510)
  * LRU：记录最近一次访问时间，建立候选池（16）；随机选取key小于池中最小的时间才会放入池中，超限则移除池中最大；
  * LFU：计算器（对应访问次数）+衰减周期（每过一定的时间，就对计算器进行减值）
  * 主动删除：随机访问key校验删除；
...
* [《Redis 的三种集群模式》](https://segmentfault.com/a/1190000022808576)
  ...
  * 集群cluster：slot数据分片；半数mater无法ping通A=A失联
...
* [《彻底搞懂Redis的线程模型》](https://juejin.cn/post/6844903970511519758)
    ...
    * 文件事件处理器使用I/O多路复用(multiplexing)程序来同时监听多个socket，并关联事件处理器；套接字准备好执行操作调用对应处理器处理；
...
>* Rdb（类似于Redis），轻量化，去除了aof之类的操作，支持Restful操作
...
* [《Spring容器启动流程》](https://blog.csdn.net/qq_35190492/article/details/110383213)
  ...
  * 创建IoC容器
    ...
    * 刷新容器↓
  * 刷新容器`refresh()`
    + 预处理->获取BeanFactory->预处理BeanFactory->子类处理->执行BeanFactory后置处理器->注册Bean后置处理器->国际化->事件派发器->子类`refresh()`->注册监听器->初始化Bean->完成初始化
...
* [《Spring Bean初始化过程》](https://www.huangchaoyu.com/2020/06/15/Spring-Bean%E5%88%9D%E5%A7%8B%E5%8C%96%E8%BF%87%E7%A8%8B/)
  ...
  * Bean初始化流程
    ...
    * （在容器刷新-初始化bean中）遍历beanNames通过反射实例化Bean，进行初始化；分为FactoryBean和普通Bean
    * 反射后通过空构造器创建Bean（完成实例化，加入二级缓存）->循环依赖处理->进行依赖注入->初始化：回调各种Aware->回调初始化方法（完成初始化）
    
  * 通过 `BeanPostProcessor#postProcessBeforeInstantiation/postProcessAfterInstantiation()` 执行Bean实例化前后操作
  
  * 通过 `BeanPostProcessor#postProcessBeforeInitialization/postProcessAfterInitialization()` 执行Bean初始化前后操作
...
* [《SpringBoot自动配置原理》](https://segmentfault.com/a/1190000018011535)
  ...
  * 过滤掉 `exclude`制定的配置类，再排除 `ConditionOnXXX` 不满足的配置类，然后导入到IOC容器中
...
* [《MVCC如何实现数据库读已提交和可重复读这两种隔离级别？》](https://blog.csdn.net/SCUTJAY/article/details/104653599)
  * ReadView：查询时生成，记录全部未提交事务；查询时检查事务id小于当前id
  * 读提交：**事务中**每次查询生成ReadView，事务中**第一次读**生成ReadView
...
+ [《InnoDB中的页合并与分裂》](https://zhuanlan.zhihu.com/p/98818611)
  ...
  + 页分裂发生在插入或更新时：页满且后一页也不足分配时进行分裂，插入新页到AB中间（逻辑上连续，物理上不连续=>错位）
  + 页合并发生在更新或删除操作中：页空置率达到一定比例时（默认50%）尝试寻找相邻页进行合并；
...
+ [《数据库事务、锁、死锁、redo log、binlog、undo log》](https://www.cnblogs.com/xuwc/p/14006112.html)
  + 二进制日志binlog刷盘时机：0(系统判断)，1(每次conmmit)，N(N个事务后)；Server层逻辑日志（SQL）
  + 重做日志redo log写入时机：0(延迟写)，1(实时写，实时刷，事务提交后刷盘)，2(实时写，延迟刷)；存储引擎层物理日志（数据页变更）
  + 二阶段提交：1.redo和undo已记录日志，事务prepare；2.binlog完成write和fsync（否则回滚），清除undo刷盘redo，事务completed
  + WAL(Write-Ahead Logging)：先写日志再写磁盘=>redo log
...
+ [《InnoDB锁与MVCC》](https://blog.csdn.net/weixin_40792878/article/details/81105848)
  ...
  + 行锁算法
    + Next-key Lock(临键锁)：默认算法，左开右闭；范围查询且**查询命中**，走非唯一索引、主键索引，锁住索引+表记录，不允许其他事务写入(解决幻读，实现RR)
...
+ [《深入学习MySQL事务：ACID特性的实现原理》](https://www.cnblogs.com/kismetv/p/10331633.html)
  ...
  + A原子性：undo log保证，旧数据(修改前)写入；C一致性：undo log, redo log；I隔离性：mvcc和锁保证；D持久性：redo log保证，新数据(修改后)写入
...
+ [《轻松理解MYSQL MVCC 实现机制》](https://blog.csdn.net/whoamiyang/article/details/51901888)
  ...
  * 查询：比当前事务版本小（或等于）的 行，删除：比当前事务版本大的行。
...
* [《HTTP 2.0 原理详细分析》](https://blog.csdn.net/zhuyiquan/article/details/69257126)
  ...
  * 多个HTTP流使用同一个TCP连接，遵守同一个流量状态控制。只要一个HTTP流遭遇到拥塞，剩下的HTTP流压根没法发出去，这就是**头部阻塞**
...
* [《SSL/TLS工作原理》](https://zhuanlan.zhihu.com/p/66029254)
  ...
  * SSL/TLS需要四次握手
  * 流程：client hello（携带可用加密套件）=>server hello（选择加密套件）=>server 下发证书=>client验证并非对称加密生成key=>server解密获取key对应随机数，后续并根据加密算法加密数据=>client/服务端告知后续加息开始加密=>对应端进行解密验证=>数据传输
...
* [《Hessian原理分析》](https://www.cnblogs.com/happyday56/p/4268249.html)
  ...
  * Hessian是一个基于 binary-RPC 实现的远程通讯 library
...
- [《RabbitMQ、Kafka、RocketMQ 是如何实现高可用的？》](https://cloud.tencent.com/developer/article/1823717)
  ...
  - rmq：（单机、普通集群）镜像集群，元数据和消息数据都会同步到其他 Broker 上进行全量备份；
...
* [《Raft 为什么是更易理解的分布式一致性算法》](http://www.cnblogs.com/mindwind/p/5231986.html)
	* 三种角色：Leader（领袖）、Follower（群众）、Candidate（候选人，如kafka的ISR）
	* 通过**随机等待重新发起投票**的方式发出投票，其他候选者还在等待中只能投票他人，得票多的获胜。
...
* [《分布式系统---幂等性设计》](https://www.cnblogs.com/wxgblogs/p/6639272.html)
	...
	* 常见保证幂等的手段：MVCC（类似于乐观锁）、去重表(唯一索引)、状态机、悲观锁、一次性token、序列号方式。 
...
+ HDFS方案：共享资源
+ kafka方案：注册controller节点时，还有记录controller_epoch纪元信息的节点，接受到小于当前纪元的数据（假死leader）时忽略
...
* [《Spring-涉及到的设计模式汇总》](https://www.cnblogs.com/hwaggLee/p/4510687.html)
  * BeanFactory=简单工厂，FactoryBean=工厂方法，AOP-Advice-MethodInterceptor=适配器，**Wrapper/Decorator=包装器，AOP=代理，Listener=观察者，实例化对象-SimpleInstantiationStrategy=策略，JdbcTemplate=模板方法
