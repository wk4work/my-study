...
* [《彻底搞懂Redis的线程模型》](https://juejin.cn/post/6844903970511519758)
    ...
    * 文件事件处理器使用I/O多路复用(multi-plexing)程序来同时监听多个socket，并关联事件处理器；套接字准备好执行操作调用对应处理器处理；
...
* [《Spring Bean初始化过程》](https://www.huangchaoyu.com/2020/06/15/Spring-Bean%E5%88%9D%E5%A7%8B%E5%8C%96%E8%BF%87%E7%A8%8B/)
  ...
  * 通过 `BeanPostProcessor#postProcessBefore/AfterInstantiation()` 执行Bean实例化前/后操作
...
* [《Elasticsearch学习，请先看这一篇！》](https://blog.csdn.net/laoyang360/article/details/52244917)
  ...
  * Elasticsearch ⇒ 索引index ⇒ 类型 ⇒ 文档 ⇒ 字段(Fields)。
* [《Elasticsearch索引原理》](https://blog.csdn.net/cyony/article/details/65437708)
  * 插入数据时为每个字段生成倒排索引，类似mysql InnoDB的辅助键索引；
  * 通过Term**前缀树**（Term太多无法全部放置内存）TermIndex定位具体Dictionary的位置，然后通过Dictionary进行二分查找；Term前缀与TermDictionary的block之间的映射关系；
  * 使用FST压缩Term Index，使用增量和Roaring bitmaps对posting list进行压缩；
  * 联合索引：使用跳表skip list与运算或者bitset按位与。
* [《ES规范 index 原则》](https://www.cnblogs.com/duanxz/p/5532313.html)
  * 指定PUT方法创建、为index添加v1后缀、type与index一对一、刷新间隔100g/d=10s、主分片数按量6、12、30；
...
* [《可扩展性设计之数据切分》](https://yq.aliyun.com/articles/38119)
	* 水平切分+垂直切分；
	* 利用中间件进行分片如，MySQL Proxy；用分片策略进行切分，如按照ID取模。 
...
* [《系统设计：关于高可用系统的一些技术方案》](https://blog.csdn.net/hustspy1990/article/details/78008324)
	...
	* 隔离：避免单一业务占用全部资源；避免业务之间的相互影响；机房隔离避免单点故障。
...
* [《限流、熔断与降级》](https://xie.infoq.cn/article/2593d1a3b9e1e06cac6502c4f)
  ...
  * 限流一般是在被调生效，熔断一般是在主调生效，降级一般的视角是从用户侧观察（整个系统）
  * 被调熔断：被调实例获取当前的实例负载情况，抛弃请求
  * 降级：由开关系统或者配置系统统一控制，有用代码自动降级的，例如接口超时降级、失败重试多次降级等
...
* [《依赖治理、灰度发布、故障演练，阿里电商故障演练系统的设计与实战经验》](https://mp.weixin.qq.com/s?__biz=MjM5MDE0Mjc4MA==&mid=2650996320&idx=1&sn=0ed3be190bbee4a9277886ef88cbb2e5)
	* 常见故障画像
	* 案例：预案有效性、故障复现、架构容灾测试、参数调优、参数调优、故障突袭、联合演练。
...
- [《RabbitMQ、Kafka、RocketMQ 是如何实现高可用的？》](https://cloud.tencent.com/developer/article/1823717)

  - rabbitMQ：（单机、普通集群）镜像集群，元数据和消息数据都会同步到其他 Broker 上进行全量备份；

  - kafka：Topic多分区，分区多副本，副本恢复机制，集群
  - RocketMq：NameServer集群（存放元数据）+Broker集群（存放队列数据）；Broker集群分为多Master、多Master多Slave、Dledger Group（1主2从，自动切换）三种；
...
* [《详谈水平分库分表》](https://www.shouxicto.com/article/2941.html)
  * 扩容可持续性，最大数据偏斜率
  * 分表法：标准的二次分片法、关系表冗余、基因法、剔除公因数法、一致性Hash法（无虚拟节点）
  * 扩容法：翻倍扩容法、一致性Hash扩容法；
...
* [《永不失联！如何实现微服务架构中的服务发现？》](https://blog.csdn.net/jiaolongdy/article/details/51188798)
  * 客户端服务发现模式：客户端直接查询注册表，同时自己负责负载均衡，如Eureka
  * 服务器端服务发现模式：客户端通过负载均衡中间服务查询服务实例，如云服务提供商的ELB
...
* [《用CAP理论来解释各个注册中心的区别》](https://cloud.tencent.com/developer/article/1616347)
  ...
  * ZK使用PAXOS算法保证一致性，Consul、ETCD、Redis使用Raft算法
...
* [《谈谈分布式事务TCC机制》](https://cloud.tencent.com/developer/article/1666011)
  * 采用的补偿机制，其核心思想是：针对每个操作，都要注册一个与其对应的确认和补偿（撤销）操作；
...
* [《深入浅出 Zookeeper 中的 ZAB 协议》](https://network.51cto.com/article/704705.html)
  * 选举投票：zid+zxid=节点id+事务id（优先），较大的投票取胜；
  * 顺序一致性：Leader发送Proposal时会扔进队列中（每个Follower对应1个队列）；
  * 强一致性：不保证，由于过半成功策略导致；

* [《【分布式】Zookeeper与Paxos》](https://www.cnblogs.com/leesf456/p/6012777.html)
  * 三角色：Leader（领袖）、Follower（群众）、Observer（**不参与Leader选举过程**，**不参与写操作**的**过半写成功**策略）
  * 消息广播：使用TCP进行通信，保证事务顺序，事务ID=ZXID；新节点进入**数据恢复模式**，从Leader中同步数据
  * 崩溃恢复：选举
  * 基本特性=>选举使用所有机器最高事务编号(ZXID)的原因
    * **确保丢弃那些只在原Leader服务器上被提出的事务**：比新Leader ZXID小
    * **确保那些已经在Leader服务器上提交的事务最终被所有服务器都提交**
  * 数据同步
    * 对象：新加入节点、选举完成未提交所有事务的节点
    * 操作：重新发送全部Proposal+Commit，完成后再加入可用节点列表(in-sync list，未同步完成节点不对外提供服务)
  * ZXID=32位epoch编号+32位计数器，选举后重置低32位，将新Leader的最大ZXID的epoch+1作为新高32位
    * 只在本机提交的原Leader因为epoch较小肯定无法成为新Leader=>完成无效Proposal抛弃
* [《集群中 Observer 的作用以及 与 Follower 的区别》](https://cloud.tencent.com/developer/article/1863322)
  * 作用：负责处理来自客户端的诸如查询数据、获取监视器等**非事务性**的会话请求操作；
  * 背景：为了解决随着集群规模增长写入性能下降（Follower过半成功策略）；
  * 跨域部署：由于Observer与Leader通信较少，可以部署在不同网络区间；
  * 底层实现
    * ObserverRequestProcessor：将事务性会话请求提交给Leader；
    * CommitProcessor：将请求放到queuedRequests（阻塞队列）中，并依次取出进行解析和封装传递给后续处理器；
    * FinalRequestProcessor：接收传递的封装对象进行处理。


#### Raft算法

> Consul、ETCD、Redis 使用

* [《Raft协议实战之Redis Sentinel的选举Leader源码解析》](https://cloud.tencent.com/developer/article/1021467)
	* 三种角色：Leader（领袖）、Follower（群众）、Candidate（候选人，leader失联时follower转化）；
	* 通过**随机等待重新发起投票**的方式重新发出投票，其他候选者还在等待中只能投票他人，得票多的获胜；
	* 假死问题：旧Leader通过较新Term降级为Follower（与zab纪元概念一致），每次选举都会term+1。
...
* [《分布式系统---幂等性设计》](https://www.cnblogs.com/wxgblogs/p/6639272.html)
  ...
  * 常见保证幂等的手段：MVCC（类似于乐观锁）、去重表（唯一索引）、状态机、悲观锁、一次性token（redis存储）、序列号方式。 
...
* [《分布式锁的几种实现方式》](http://www.hollischuang.com/archives/1716)
  ...
  * Zookeeper 分布式锁：通过**有序临时节点**实现锁机制，自己对应的节点需要最小，则被认为是获得了锁。优点：集群可以透明解决单点问题，避免锁不被释放问题，同时锁可以重入。缺点：性能不如缓存方式，吞吐量会随着zk集群规模变大而下降。
...
+ [《MySql主从复制及延迟》](https://blog.csdn.net/gogokongyin/article/details/110238534)
  + 半同步复制：主完成读同步（任意从节点）后再结束写请求
  + 多库并行复制：MySql 5.6支持
  + 多表并行复制：异步GTID复制
  + 其他策略
    + 利用中间件进行读写分发：写更新时间窗内将读请求路由到主库上
    + 利用缓存：与上基本类似，通过检查key有没有对应数据路由到主库上

+ [《缓存和数据库一致性问题，看这篇就够了》](http://kaito-kidd.com/2021/09/08/how-to-keep-cache-and-consistency-of-db/)
  + 不考虑数据库同步延迟：更新数据库=>删缓存
  + 考虑数据库同步延迟：延时双删策略，删缓存=>更新数据库=>删缓存（延迟时间>主从同步时间）
  + 缓存删除失效：重试机制，消息队列自消费或者使用MySQL中间件Canal监控Binlog

* [《分布式系统：数据一致性解决方案》](https://cloud.tencent.com/developer/article/1763155)
  * 理论：CAP & BASE
  * 重试机制：幂等，同步使用异常，异步使用消息
  * 分布式事务：2PC、3PC、TCC、事务管理器（组件、应用）
  * DB和MQ数据一致性：通过可靠性投递（confirm）和可靠性消费、持久化保障（ack）
  * DB和Cache数据一致性：重试更新
  * 兜底核对
  * jraft框架
...
> 假死场景：主节点临时宕机，从节点重新选举主节点，原主节点恢复后集群有2个主节点；主要通过年代的机制解决；
>
> 假死解决方案：通过类似过期的概念，使原Leader落后当前版本重新进行同步；

### 脑裂

> 脑裂场景：由于集群网络通信导致的节点间无法互相访问，从而出现2个提供服务的网络分区；主要通过法定人数的机制解决；
> 
> 脑裂主要解决方案
>
...
+ HDFS方案：共享资源
+ [《kafka系列（Controller 脑裂问题）》](https://zhuanlan.zhihu.com/p/75524641)
  + 抢占式注册controller节点通过zk临时有序节点，session timeout时触发；
  + 还有记录controller_epoch纪元信息的节点，接受到小于当前纪元的数据（假死leader）时忽略

+ Elasticsearch方案：法定人数，提供参数【minimum_master_nodes】来解决脑裂问题
...
* [《深入浅出 TCP/IP 协议》](https://www.cnblogs.com/onepixel/p/7092302.html)
  ...
  * 网络层(IP)：定义网络地址，区分网段，子网内MAC寻址，对于不同子网的数据包进行路由；
  * 传输层(TCP/UDP)：定义端口，标识应用程序身份，实现端口到端口的通信；
  * 应用层(HTTP/SSL/RPC)：定义数据格式并按照对应的格式解读数据。
* [《TCP协议中的三次握手和四次挥手》](https://blog.csdn.net/whuslei/article/details/6667471/)
  * 防止已失效的连接请求报文段突然又传送到了服务端；服务端的ack和syn可以合并；
  * 服务端客户端都有需要发送的最后数据，因此ack和fin不能合并。
...
* [《图解 TCP 重传、滑动窗口、流量控制、拥塞控制》](https://zhuanlan.zhihu.com/p/135932018)
  ...
  * 拥塞控制：避免「发送方」的数据填满整个网络，超时重传=拥塞，实际窗口=min（拥塞窗口，接收窗口）
    * 算法：慢启动（cwnd+1）=>拥塞避免（cwnd+1/N）=>拥塞发生（超时重传cwnd=1，快速重传cwnd=cwnd/2）=>快速恢复=>拥塞避免（主要根据拥塞窗口算法进行区分）
...
* [《HTTP 2.0 原理详细分析》](https://blog.csdn.net/zhuyiquan/article/details/69257126)
  * 利用二进制帧负责传输、多路复用、优先级、服务端推送
  * **头部阻塞**：多个HTTP流使用同一个TCP连接，遵守同一个流量状态控制。只要一个HTTP流遭遇到拥塞，剩下的HTTP流压根没法发出去

### HTTPS
* [《HTTPS原理分析》](https://blog.leapmie.com/archives/418/#%E4%B8%BA%E4%BB%80%E4%B9%88%E6%95%B0%E6%8D%AE%E4%BC%A0%E8%BE%93%E6%98%AF%E7%94%A8%E5%AF%B9%E7%A7%B0%E5%8A%A0%E5%AF%86%EF%BC%9F)
	* 使用非对称加密协商加密算法（防篡改），使用对称加密方式传输数据（效率高，单秘钥+硬件加速）
	* 使用第三方机构签发的证书，来加密公钥，用于公钥的安全传输、防止被中间人篡改。
* [《SSL/TLS工作原理》](https://zhuanlan.zhihu.com/p/66029254)
  * 非对称加密可能存在拦截公钥后替换秘钥篡改数据导致真实性问题=>使用CA进行验证
  * CA证书集成在了浏览器和操作系统里面不会被劫持
  * SSL/TLS需要四次握手
  * 流程：client hello（携带可用加密套件）=>server hello（选择加密套件）=>server 下发证书=>client验证并非对称加密生成key=>server解密获取key对应随机数，后续并根据加密算法加密数据=>client/服务端告知后续消息开始加密=>对应端进行解密验证=>数据传输

## 网络模型

* [《web优化必须了解的原理之I/o的五种模型和web的三种工作模式》](http://blog.51cto.com/litaotao/1289790)
	* 五种I/O模型：阻塞I/O，非阻塞I/O，I/O复用、事件(信号)驱动I/O、异步I/O，前四种I/O属于同步操作，I/O的第一阶段（内核读取磁盘）不同、第二阶段（内核拷贝数据至线程内存）相同，最后的一种则属于异步操作。
...
* [《select、poll、epoll之间的区别比较》](https://zhuanlan.zhihu.com/p/345762559)
  ...
  * 连接数：select受机器限制（默认32位1024，64位2048）；poll无限制（链表）；epoll超大上限（1G内存10w连接）
  * 效率：
    * select/poll时间复杂度O(n)，遍历整个fd集合；知道需要进行读写，但是不知道具体是哪个流，所以需要无差别轮询（线性扫描Socket）
    * epoll时间复杂度O(1)，查询就绪列表；事件驱动：能通知（回调）哪个流触发了什么事件
  * 消息传递：select/poll每次用户态内核态切换需要拷贝，epoll通过共享一块内存避免复制只要一次拷贝（注册回调）
  * 性能：poll会随着并发增加，性能逐渐下降，epoll采用红黑树结构，性能稳定，不会随着连接数增加而降低。
...
* [《两种高效的服务器设计模型：Reactor和Proactor模型》](https://blog.csdn.net/u013074465/article/details/46276967)
  * reactor-单线程：一个reactor线程处理全部，多路分离socket，Accept新连接，并分派请求到处理器链；
  * reactor-多线程单reactor：在事件处理器（Handler）链部分采用了多线程（线程池）
  * reactor-多线程多reactor：将监听并accept新连接交由mainReactor负责；socket通过多路复用器（Acceptor）分派给subReactor；subReactor负责多路分离已连接的socket，读写网络数据


### Epoll

* [《epoll使用详解（精髓）》](https://www.cnblogs.com/fnlingnzb-learner/p/5835573.html)
  * epoll_create：创建epoll，消耗一个fd
  * epoll_ctl：注册事件，确定监听事件类型
  * epoll_wait：等待事件
...
* [《深入理解Java NIO》](https://www.cnblogs.com/geason/p/5774096.html)
  ...
  * Buffer进行缓冲区读写，channel面向链接，Selector代替主线程进行轮询
...
* [《Hessian原理分析》](https://www.cnblogs.com/happyday56/p/4268249.html)
  * binary-RPC：标准格式二进制文件，通过转换为流进行传输，传输协议HTTP
  * Hessian是一个基于 binary-RPC 实现的远程通讯框架
...
> 应用场景：广告相关实时统计、推荐系统用户画像标签实时更新、线上服务健康状况实时监测、实时榜单、实时数据统计。
...
* [《用通俗易懂的话说下hadoop是什么，能做什么》](https://blog.csdn.net/houbin0912/article/details/72967178)
  * Hadoop=HDFS+Mapreduce；开源框架；分布式计算的解决方案
...
* [《用通俗易懂的大白话讲解Map/Reduce原理》](https://blog.csdn.net/oppo62258801/article/details/72884633)
  * 是一种编程模型，是一种编程方法、抽象理论，是一种分布式计算框架
...
+ cpu100：[《linux java cpu 100%问题排查》](https://blog.csdn.net/Great_Smile/article/details/104578994)
  + `ps -ef | grep name`查找进程=>`top -Hp pid` 查找高cpu占用线程=>`jstack pid | grep tid`查看线程运行；
  + 使用阿里的arthas、通过dump stack
+ 请求无响应
  + 确认服务是否正常启动=>检查网络连接状态=>检查防火墙设置=>检查负载均衡器和代理服务器=>检查服务配置=>检查系统硬件资源=>开启调试模式：如果以上方法仍然无法解决问题，可以开启调试模式，打印更详细的日志信息或调试信息，从而定位问题。
...
> KVM的优点：
>
> 1. 硬件隔离：KVM提供完整的硬件隔离，每个虚拟机可以访问独立的硬件资源，比如CPU、内存、网络接口、存储等。这样可以避免不同虚拟机之间互相干扰。
> 2. 虚拟化技术成熟：KVM是一种成熟的虚拟化技术，可以在多种不同的硬件和操作系统上运行。
> 3. 支持多种操作系统：KVM可以同时运行多个不同的操作系统，包括Windows和Linux等。

> KVM的缺点：
>
> 1. 资源消耗高：每个虚拟机都需要一定的系统资源，比如内存、CPU、硬盘空间等。因此，如果同时运行多个虚拟机，会消耗大量的资源，导致性能下降。
> 2. 启动时间长：启动一个虚拟机需要一定的时间，这样会影响整个系统的启动时间。
...
> Docker的优点：
>
> 1. 轻量级：Docker容器是轻量级的，可以在不同的主机上快速创建、销毁和迁移，不会占用大量的资源。
> 2. 快速启动：由于Docker容器是基于镜像的，因此可以在几秒钟内启动容器，而不需要像虚拟机一样等待一段时间。
> 3. 易于管理：Docker提供了一套完整的工具链，可以方便地管理容器，比如创建、启动、停止、重启、删除等操作。

> Docker的缺点：
>
> 1. 不支持完整的硬件隔离：Docker容器之间共享主机的操作系统内核和硬件资源，因此无法提供完整的硬件隔离，可能存在容器之间的资源竞争问题。
> 2. 仅支持Linux操作系统：Docker容器只能运行在Linux操作系统上，无法在Windows操作系统上运行。
> 3. 镜像构建较复杂：构建Docker镜像需要一定的技术水平，需要理解Dockerfile和Docker镜像的概念。

* [《几张图帮你理解 docker 基本原理及快速入门》](https://www.cnblogs.com/SzeCheng/p/6822905.html)
  * 优势：更快的交付、更高效的资源使用、更简单的管理
  * 核心概念：镜像、仓库、容器
...
> 容器编排平台，通过和容器引擎（非必需Docker）集成来实现容器化。
>
> 特色：容器化应用程序、容器编排、资源调度、健康检查、存储管理。
>
> 容器化应用程序通常使用Docker或其他容器引擎（关键组件）来打包应用程序和依赖项，并将其打包到一个或多个容器镜像中。
> 容器引擎是通过容器运行时接口（Container Runtime Interface，CRI）与Kubernetes集成的。CRI定义了一组标准化的API，用于在容器引擎和Kubernetes之间进行通信。
...
* [《xss攻击原理与解决方法》](https://blog.csdn.net/qq_21956483/article/details/54377947)
  * 对网页注入可执行代码且成功地被浏览器执行，反射型：诱导点击其他网站，存储型：在服务器保存恶意代码
  * 防范：请求参数过滤特殊字符
### CSRF
* [《CSRF原理及防范》](https://coderxing.gitbooks.io/architecture-evolution/di-san-pian-ff1a-bu-luo/641-web-an-quan-fang-fan/6412-csrf.html)
  * 构造的第三方页面诱导受害者完成加载或者点击，利用受害者的权限，以其身份向合法网站发起恶意请求
  * 防范：添加 Referer 域名白名单，令牌 (Token )验证
...
* [《SQL注入》](https://coderxing.gitbooks.io/architecture-evolution/di-san-pian-ff1a-bu-luo/641-web-an-quan-fang-fan/6413-sql-zhu-ru.html)
  * 伪造请求参数，执行恶意语句；防范：预编译语句（PreparedStatement）
...
### DDoS 防范
* [《学习手册：DDoS的攻击方式及防御手段》](http://blog.nsfocus.net/analysis-ddos-attack-defense/)
  * DDoS（分布式拒绝服务）：利用分布式的客户端，向目标发起大量看上去合法的请求，消耗或者占用大量资源，从而达到拒绝服务的目的
  * SYN洪水攻击：发送了大量的TCP SYN报文，使服务器在短时间内产生大量的半开连接占满连接表，导致无法建立新的TCP连接
  * 防范：硬件保障、异常流量的清洗、集群防御
...
* [《常见非对称加密算法》](https://coderxing.gitbooks.io/architecture-evolution/di-san-pian-ff1a-bu-luo/642-shu-ju-jia-mi/6424-chang-yong-fei-dui-cheng-jia-mi-suan-fa.html)
	* 公钥加密，私钥解密；或者相反
	* RSA(取名自发明三位科学家首字)、DSA(Digital Signature Algorithm，数字签名算法)、ECDSA(Elliptic Curve Digital Signature Algorithm，螺旋曲线加密算法)
	* 和 RSA 不同的是 DSA 仅能用于数字签名，不能进行数据加密解密，其安全性和RSA相当，但其性能要比RSA快。
* [《区块链的加密技术》](https://blog.csdn.net/wu2374633583/article/details/79174547)
  * 比特币私钥：非对称加密=>SHA256 哈希算法=>Base58 转换，公钥：非对称加密=>SHA256、RIPEMD160双哈希算法摘要（Hash160的结果）=>SHA256 哈希算法=>Base58 转换
...
* [《WEB应用的安全的登录认证》](https://blog.51cto.com/jamesfancy/1161210)
  * 密文传输：通过用户名U获取加盐salt和随机数R，使用过该数据进行二次加密；服务端根据U获取salt反向处理验证；传输U、salt、R和二次加密密文；
  * 明文传输：使用非对称加密，前端通过公钥加密传输数据，后端根据私钥解密数据；
...
> 1. 架构和设计模式：需要熟悉常见的架构和设计模式，例如分层架构、微服务架构、容器化、领域驱动设计、RESTful API 等。需要了解它们的特点、优缺点、适用场景等。
> 2. 云计算和大数据：需要了解云计算和大数据技术的基本概念、原理和应用，例如云计算平台、分布式计算、分布式存储、大数据处理框架等。
> 3. 数据库和缓存：需要熟悉数据库和缓存的基本概念、特点、优缺点和使用场景，例如 MySQL、Redis、MongoDB 等。
> 4. 消息中间件和队列：需要了解消息中间件和队列的基本概念、特点、优缺点和使用场景，例如 RabbitMQ、Kafka 等。
> 5. 网络和协议：需要了解网络和协议的基本概念和原理，例如 TCP/IP、HTTP、HTTPS 等。
> 6. 安全和性能：需要了解安全和性能方面的知识，例如身份认证、授权、数据加密、性能优化等。
> 7. 工具和技术栈：需要了解常用的开发工具和技术栈，例如 IDE、Git、Jenkins、Docker、Kubernetes 等。
> 8. 沟通和协作能力：需要具备良好的沟通和协作能力，可以准备一些团队协作和沟通方面的问题，例如如何与团队沟通、如何处理冲突、如何协调不同团队之间的工作等。

* [《架构师画像》](https://cloud.tencent.com/developer/news/284788)
  * 业务理解和抽象能力
  * NB的代码能力
  * 全面：1. 在面对业务问题上，架构师脑海里是否会浮现出多种技术方案；2. 在做系统设计时是否考虑到了足够多的方方面面；
  * 全局：是否考虑到了对上下游的系统的影响。
  * 权衡：权衡投入产出比；优先级和节奏控制；
  * 产品本身=>整体的方案（业务解决方案）=>商业价值。
* [《关于架构优化和设计，架构师必须知道的事情》](http://www.infoq.com/cn/articles/architecture-optimization-and-design-the-architect-must-know)
  * 细节：模块化、轻耦合、高聚合、无共享、无状态架构；
  * 减少各个组件之前的依赖、注意服务之间依赖所有造成的链式失败及影响等。
  * 应急基础设施、配置、测试、开发、运维综合考虑。
  * 考虑人、团队、和组织的影响。
...
* [《消息队列原理和选型》](https://juejin.cn/post/7096095180536676365)
  * 基于功能特性（可靠性、持久化、广播、分布式事务、消息顺序）、性能（吞吐量、延迟、负载均衡、容错性）、可扩展性、社区支持、成本考虑（硬件、运维、开发）；
  * Kafka：高吞吐，不支持延迟、重试、路由，常用于日志和海量数据传输；
  * RocketMQ：高吞吐，默认持久化可靠性最强，常用于金融场景；
  * RabbitMQ：性能最全面，吞吐一般，默认选型；
* [《存储组件选型》](https://z.itpub.net/article/detail/5ED6D90DFC0588ADEDCF87E0FB5A3EED)
  * 原则：场景、数据规模、掌控度（DEV+DBA）；
  * Relational（MySQL）：事务、稳定、成熟；不易水平扩展、结构复杂、统计检索能力弱；
  * KV（Redis）：读写效率高、速度快；硬件成本；
  * Search engine（ES）：分词全文检索、聚合、数据量；延迟、硬件成本；
  * Document（MongoDB）：易于字段和水平扩展、性能高；不易维护、认知成本高；
  * Wide column（HBase）：列结构、数据量、扩展、成本低、聚合计算；认知成本高、只有行事务；
* [《流式计算技术选型》](https://juejin.cn/post/6885264924936208391)
  * Flink
    * 优点：支持批处理和流式计算；高性能、低延迟，适用于实时数据处理；提供了易用的API和丰富的运算符，支持复杂的数据处理和分析；中文社区友好；
    * 缺点：学习成本高、部署管理比较复杂；
  * Storm：简单易用，高可用性，社区活跃；性能较差、延迟较高，复杂逻辑需要手动编写，可扩展性一般，无状态管理机制，不支持准确一次语义；
  * Spark Steaming
    * 优点：高性能，支持实时数据处理，与 Apache Spark 集成，生态系统丰富；
    * 缺点：伪实时（微小批量）延迟高，不适用于小数据量场景，学习成本高；
...
* [《Spring-涉及到的设计模式汇总》](https://www.cnblogs.com/hwaggLee/p/4510687.html)
  * BeanFactory=简单工厂，FactoryBean=工厂方法，AOP-Advice-MethodInterceptor=适配器，DemoWrapper/Decorator=包装器，AOP=代理，Listener=观察者，实例化对象-SimpleInstantiationStrategy=策略，JdbcTemplate=模板方法
...
- 典型应用：Java8 Lambda表达式

> 优点：
>
> 1. 易于并发编程：函数式编程中的函数通常是无状态的，不会修改函数外的数据，这使得函数式编程更易于并发编程。
> 2. 可重用性高：函数式编程中的函数通常是单纯的、可组合的，这使得它们易于重用，可以在多个应用程序中进行共享。
> 3. 更少的副作用：函数式编程中的函数通常是纯函数，它们不会对函数外的变量造成副作用，这有助于减少错误并提高代码可读性。
> 4. 更少的代码：函数式编程通常使用更少的代码来完成相同的任务，因为函数式编程的函数通常是小而简单的。

> 缺点：
>
> 1. 学习曲线较陡：函数式编程中的概念和技术与传统的命令式编程有很大不同，这使得函数式编程对于初学者来说可能较为困难。
> 2. 性能不如命令式编程：函数式编程的编译器和解释器通常比命令式编程的编译器和解释器慢，这使得函数式编程在某些情况下可能性能不如命令式编程。
> 3. 可变性受限：函数式编程强调不可变性，这使得它不适用于需要频繁更新的场景。

### 面向对象编程

> 面向对象编程优点：封装性、可扩展性、易于理解、代码重构；
>
> 面向对象编程缺点：复杂性、可变性、并发编程；
...
* [《Spring AOP JDK和CGLIB动态代理的区别》](https://www.jianshu.com/p/84a3c71b518b)
	* 如果目标对象的实现类实现了接口，采用 JDK 动态代理；如果目标对象的实现类没有实现接口，采用 CGLIB
...
> 常见的领域划分方法：
>
> 1. 模块划分：根据业务功能和业务流程划分领域，例如订单、库存、用户等。
> 2. 问题域划分：将领域按照实际业务中涉及的问题进行划分，例如财务、物流、售后等。
> 3. 关注点划分：根据业务领域中各个参与者（例如用户、管理员、客服等）的关注点进行划分，例如用户管理、订单管理、库存管理等。
> 4. 业务线划分：根据不同的业务线划分领域，例如电商业务、金融业务、医疗业务等。

> 在进行领域划分时，需要注意以下几点：
>
> 1. 领域应该是业务上具有独立性的，避免出现多个领域处理同一业务的情况。
> 2. 领域之间应该通过明确的接口和协议进行交互，避免出现耦合度过高的情况。
> 3. 领域划分应该根据实际业务需求和领域的规模、复杂度等因素进行，避免过度设计。
> 4. 领域划分应该是可扩展的和可变化的，以适应业务变化和需求的变化。
...
> 1. 客户端-服务器（Client-Server）：客户端向服务器发送请求，服务器进行处理并返回响应。这种模式适用于分布式应用，可以将客户端与服务器分开部署，提高了系统的可伸缩性和可用性。
> 2. 分层（Layered）：将系统划分为多个层次，每个层次都有特定的功能。这种模式使得系统更易于维护和修改，同时也有助于实现可重用性和可扩展性。
> 3. MVC（Model-View-Controller）：将应用程序分为三个部分：模型、视图和控制器。模型负责数据处理，视图负责呈现数据，控制器负责协调模型和视图之间的交互。这种模式使得应用程序更易于维护和扩展，同时也提高了代码的可重用性。
> 4. 微服务（Microservices）：将应用程序拆分为多个小型服务，每个服务都运行在独立的进程中，并且可以使用不同的编程语言和技术栈。这种模式使得系统更易于扩展和维护，同时也有助于实现高可用性和弹性。
> 5. 事件驱动（Event-Driven）：应用程序通过事件进行通信和交互。当某个事件发生时，系统中的其他组件会根据事件类型采取相应的行动。这种模式使得系统更具有弹性和可扩展性，同时也提高了系统的可维护性。
> 6. 服务导向架构（Service-Oriented Architecture，SOA）：将应用程序分解为一组可重用的服务，这些服务是相互独立的，通过标准化接口进行通信。
> 7. 无服务器架构（Serverless Architecture）：将应用程序开发和运行的责任交给云服务提供商，开发人员只需要关注业务逻辑，无需关注底层的基础设施。
...
- [《SOA架构和微服务架构的区别》](https://segmentfault.com/a/1190000040618603)
  - 微服务架构相比SOA架构更加灵活、可扩展和易于维护，但同时也需要更多的管理和协调工作；
  - 服务粒度：微服务粒度更小，对应具体功能；SOA高耦合，对应更大的业务；
  - 通信方式：微服务通常采用轻量级协议如HTTP，SOA采用重量级如SOAP；
  - 数据管理方式：微服务中每个应用可能存储自己的数据，SOA集中存储共享数据；
  - 部署方式：微服务独立部署，SOA通常集中部署。

### 云原生

#### Serverless

> 云计算架构模式，基于事件驱动计算（Event-driven Computing）和函数即服务（Function-as-a-Service，FaaS）等概念，使开发人员可以在不需要管理服务器和基础架构的情况下编写和部署应用程序。

* [《什么是Serverless无服务器架构？》](http://www.jdon.com/soa/serverless.html)
	* Serverless 不代表去除服务器，而是去除对服务器运行状态的关心。
	* Serverless 代表一思维方式的转变，从“构建一套服务在一台服务器上，对多个事件进行响应”转变为“构建一个服务，来响应一个事件”。
	* Serverless 不代表某个具体的框架。

* [《Serverless的优缺点》](https://jimmysong.io/serverless-handbook/concepts/why-serverless.html)
	* 优点IaaS：降低运营成本（基础组件交由云服务提供商）、降低开发成本、扩展性、简单性、资源利用率;
	* 优点FaaS：降低人力成本（只负责业务组件）、降低风险（不关注非业务组件）、降低开销（弹性扩容）、降低迭代周期；
	* 缺点：无状态管理、网络延迟、本地测试困难；
	

#### IaaS、FaaS、SaaS

> 云计算的服务模式，Infrastructure as a Service基础设施即服务，Function as a Service函数即服务，Software as a Service是指软件即服务。

#### Service Mesh

> 用于管理和监控微服务之间的通信，提供服务发现、流量管理、安全性、故障处理等功能，旨在简化微服务架构的复杂性。

* [《什么是Service Mesh？》](https://time.geekbang.org/article/2355)
  * 基础设施层，用于服务通信，处于TCP/IP上的抽象层；为应用运行时提供统一的、应用层面的可见性和可控性
...
> 1. 批量发送：减少网络通信的开销和网络带宽的占用，调整生产者的 batch.size 和 linger.ms 参数来控制批量发送的大小和时间窗口。
> 2. 异步发送：发送消息的同时执行其他操作，例如对消息进行序列化、压缩等处理，减少发送消息的时间开销。
> 3. 重用生产者实例：避免频繁创建和销毁生产者实例，将生产者实例封装为单例或者连接池。
> 4. 增加分区数：增加分区数来提高并行处理的能力，但是会增加管理和维护的复杂度。
> 5. 调整 acks 参数：0=不需要等待任何确认，可能会出现消息丢失的情况；1=需要等待分区的 Leader 确认消息，可以提高可靠性；all=等待 ISR 中的所有副本确认消息。

#### kafka消费端提效

> 1. 增加消费者实例；
> 2. 调整消费者配置：例如 fetch.min.bytes 和 fetch.max.wait.ms 等。
> 3. 手动提交偏移量：避免频繁提交偏移量带来的性能问题，同时可以控制消费的粒度。
> 4. 批量消费：设置 fetch.max.bytes 参数来控制批量消费的大小；使用 poll 方法进行批量消费，通过调整max.poll.records。
> 5. 提高网络带宽
> 6. 提高消息处理效率：消费端逻辑简化。
...
* [《架构之重构的12条军规》](http://www.infoq.com/cn/articles/architect-12-rules-complete/)
  * 确定重构的目的和必要性、定义“重构完成”的界限、渐进式重构、确定当前的架构状态
  * 不要忽略数据、管理好技术债务、以目标为导向、做好准备面对压力
  * 了解业务、做好面对非技术因素的准备、对于代码质量有所掌握、让团队做好准备。
