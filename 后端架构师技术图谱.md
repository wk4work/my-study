...
+ 堆排序：建立最小堆（Top Max）或者最大堆（Top Min），循环添加元素并调整保证继续为堆
...
* [《硬核系列 | 深入剖析 Java 协程》](https://xie.infoq.cn/article/cef6d2931a54f85142d863db7)
  ...
  * 协程需要主动让出占用
  * 目前java三分库通过保存线程栈帧信息然后重新读取的方式进行协程的上下文切换
## Linux
...
* [《彻底理解synchronized》](https://juejin.cn/post/6844903600334831629)
  ...
  + 锁升级：栈中会存储锁记录；CAS更新对象头markword失败会升级为重量级锁
...
* [《select、poll、epoll之间的区别比较》](https://zhuanlan.zhihu.com/p/345762559)
  ...
  * epoll-et：读到没数据或者报错，只触发一次事件；epoll-lt（默认模式）：一直读重复触发事件
...
+ [《SpringMVC 框架理解（详解）》](https://zhuanlan.zhihu.com/p/128103535)
  ...
  + 角色划分：前端控制器（DispatcherServlet）、请求到处理器映（HandlerMapping）、处理器适配器（HandlerAdapter）、视图解析器（ViewResolver）、处理器或页面控制器（Controller）、验证器（ Validator）、命令对象（Command 请求参数绑定到的对象就叫命令对象）、表单对象（Form Object 提供给表单展示和提交到的对象就叫表单对象）
  + 与Struts2对比
