...
+ [《深入理解Java类加载器(ClassLoader)》](https://blog.csdn.net/javazejian/article/details/73413292)
  + 流程：加载（创建Class对象）->链接【验证（文件是否安全）->准备（类变量分配方法区内存并赋初始值）->解析（常量池中的符号引用替换为直接引用（指针））】->初始化（类变量赋值）
...
+ [《详解 HashMap 数据结构（JDK8）》](https://juejin.cn/post/6844904111817637901)
  + 数组+链表+红黑树（加快链表的查询效率）
  + Hash冲突解决方案：**链地址法**（hash 值相同时，则会将他们置于数组的同一个位置处，并以链表的形式呈现）（另外一种开放地址法：哈希完后如果有冲突，则按照某种规则找到空位插入）
...
* [《你以为你真的了解final吗？》](https://juejin.cn/post/6844903601068998664)
  ...
  + 读重排序：禁止初次**读对象的引用**与**读该对象包含的final域**的重排序
...
+ [《并发容器之ThreadLocal》](https://juejin.cn/post/6844903602436177933)
  ...
  + ThreadLocalMap是一个Entry数组；Entry继承了WeakReference弱引用；ThreadLocalMap 使用**开发地址法**解决散列冲突
...
+ cpu100：[《linux java cpu 100%问题排查》](https://blog.csdn.net/Great_Smile/article/details/104578994)
  + TOP查找进程=>TOP查找线程=>jstack查看线程运行；使用阿里的arthas；通过dump stack
...
* [《Redis底层存储模型》](https://i6448038.github.io/2019/12/01/redis-data-struct/)
	* 字符串：int、sds，列表：ziplist、双向链表，hash：ziplist、哈希表，set：intset、哈希表，zset：ziplist、跳表（根据长度、元素长度确定类型）
	* 使用 ziplist 存储链表，ziplist是一种压缩链表，它的好处是更能节省内存空间，因为它所存储的内容都是在连续的内存区域当中的。
	* 使用 skiplist(跳跃表)来存储有序集合对象、查找上先从高Level查起、时间复杂度和红黑树相当，实现容易，无锁、并发性好。
* [《Redis持久化方式》](http://doc.redisfans.com/topic/persistence.html)
	* RDB方式：定期备份快照，常用于灾难恢复。优点：通过fork出的进程进行备份，不影响主进程、RDB 在恢复大数据集时的速度比 AOF 的恢复速度要快。缺点：会丢数据。
	* AOF方式：保存操作日志方式。优点：恢复时数据丢失少，缺点：文件大，回复慢；可以自动重建压缩命令（不影响数据）；AOF的刷盘时机与MySQL RedoLog类似
...
* [《Redis 的三种集群模式》](https://segmentfault.com/a/1190000022808576)
  * 主从复制：故障处理需要人工干预
  * 哨兵：主观下线（单节点发现）=>客观下线（半数多节点确认）=>通知slave=>升级slave至master
...
* [《Redis集群原理详解》](https://blog.csdn.net/a745233700/article/details/112691126)
  ...
  * 故障：主观下线（单节点发现）=>客观下线（半数多节点确认）=>从节点选举
...
+ [《InnoDB锁与MVCC》](https://blog.csdn.net/weixin_40792878/article/details/81105848)
  ...
  + undo log: 保证原子性，旧数据(修改前)写入
  + redo log: 保证持久性，新数据(修改后)写入
...
* [《HTTP的发展》](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Basics_of_HTTP/Evolution_of_HTTP)
  ...
  * http 1.1：标准化，连接复用、管线化技术、内容协商机制、HOST支持同ip多域名
  * http 2.0：增强表现，复用、压缩头、服务器推送
...
* [《select、poll、epoll之间的区别比较》](https://zhuanlan.zhihu.com/p/345762559)
  * 连接数：select受机器限制；poll无限制（链表）；epoll超大上限（1G内存10w连接）
  * 效率：
    * select/poll时间复杂度O(n)；知道需要进行读写，但是不知道具体是哪个流，所以需要无差别轮询（线性扫描Socket）
    * epoll时间复杂度O(1)；事件驱动：能通知哪个流触发了什么事件
  * 消息传递：select/poll每次用户态内核态切换需要拷贝，epoll通过共享一块内存避免复制
...
+ [~~《MySQL主从同步延迟问题，数据一致性问题》~~](http://blog.zhangwu.vip/archives/934)
  + 缓存数据库一致性
    + 先删缓存再更新数据库：延时双删策略，删缓存=>更新数据库=>删缓存
